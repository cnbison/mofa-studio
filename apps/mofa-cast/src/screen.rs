...
live_design! {
                            ...
                            volume_slider = <Slider> {
                                ...
                                draw_bg: {
                                    fn pixel(self) -> vec4 {
                                        ...
                                        // Filled track (blue, based on slide_pos)
                                        let fill_width = self.rect_size.x * self.slide_pos;
                                        sdf.box(0., 0., fill_width, self.rect_size.y, 6.0);
                                        sdf.fill(#3b82f6);

                                        // Thumb (circle at slide_pos position)
                                        let thumb_x = self.rect_size.x * self.slide_pos;
                                        let thumb_y = self.rect_size.y * 0.5;
                                        sdf.circle(thumb_x, thumb_y, 8.0);
    ...
    exported_audio_path: None  // No audio exported yet
    audio_playback_process: None  // No playback running
    is_playing: false  // Not playing
    volume_level: 100.0  // 100% volume by default
}
...
impl Widget for CastScreen {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        // Volume slider
        if let Some(volume) = self.view.slider(ids!(main_content.right_panel.content_area.audio_player_section.volume_control.volume_slider)).value() {
            self.volume_level = volume as f32;
            ::log::info!("Volume changed to: {}", volume);

            // Update status to show new volume
            if self.is_playing {
                self.view.label(ids!(main_content.right_panel.content_area.audio_player_section.player_status))
                    .set_text(cx, &format!("Playing (Volume: {:.0}%)", volume));
            }
        }
    ...
    fn handle_play_audio(&mut self, cx: &mut Cx) {
        if let Some(ref audio_path) = self.exported_audio_path {
            if audio_path.exists() {
                ...
                // Use the actual volume level (should be 100.0 by default)
                let volume = self.volume_level;

                ::log::info!("Playing with volume: {}", volume);

                let result = std::process::Command::new(player)
                    .arg("-v")  // Set volume (afplay specific)
                ...
                match result {
                    Ok(mut child) => {
                        ...
                        self.view.label(ids!(header.header_description))
                            .set_text(cx, &format!("▶️ Playing: {} (Volume: {}%)",
                                audio_path.file_name().unwrap_or_default().to_string_lossy(),
                                volume));

                        self.view.label(ids!(main_content.right_panel.content_area.audio_player_section.player_status))
                            .set_text(cx, &format!("Playing (Volume: {}%)", volume));

                        ::log::info!("Started playback: {} with volume {}", audio_path.display(), volume);
                    }
                    Err(e) => {
                        self.view.label(ids!(header.header_description))
